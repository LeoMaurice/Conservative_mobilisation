...
cp <- corpus(filtered_data$text, 
             docvars = filtered_data |> select(Asso, Date, Type_doc, Idéologie,Droite) |> as.data.frame(), 
             docnames = filtered_data$ID)

# too frequent words
too_frequent_words <- c("femme","homme","personne","pouvoir","sexe","genre")
# words to remove
...
stm_lda <- stm(dfm,
               K=num_topic, 
               prevalence = ~ Droite + factor(Idéologie), #+ factor(Asso),
               #content =~ ,
               init.type = "Spectral",
               seed = seed, 
               verbose = FALSE)
...
effets <- estimateEffect(formula = 1:num_topic ~ Droite + factor(Idéologie), 
